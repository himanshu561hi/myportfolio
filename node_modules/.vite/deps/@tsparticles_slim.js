...
function isPointInside(point, size, offset, radius, direction) {
  const minRadius5 = 0;
  return areBoundsInside(calculateBounds(point, radius ?? minRadius5), size, offset, direction);
}
...
function drawEmoji(data, image) {
  const { context, opacity } = data, half10 = 0.5, previousAlpha = context.globalAlpha;
  if (!image) {
    return;
  }
  const diameter = image.width, radius = diameter * half10;
  context.globalAlpha = opacity;
...
var EmojiDrawer = class {
  ...
  particleInit(_container, particle) {
    const double12 = 2, shapeData = particle.shapeData;
    if (!shapeData?.value) {
      return;
    }
    ...
    const padding = emojiOptions.padding * double12, maxSize = getRangeMax(particle.size.value), fullSize = maxSize + padding, canvasSize = fullSize * double12;
    let image;
    if (typeof OffscreenCanvas !== "undefined") {
      ...
      context.font = `400 ${maxSize * double12}px ${font}`;
...
var RotateUpdater = class {
  ...
  init(particle) {
    ...
    if (rotateDirection === RotateDirection.random) {
      const index = Math.floor(getRandom() * double10), minIndex2 = 0;
